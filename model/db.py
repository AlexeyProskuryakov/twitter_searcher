...
class db_handler(database):
    ...
    def save_user(self, ser_user):
        ...
        """
        log.debug('saving user: un: %s;' % (ser_user['name_']))
        son = SON(ser_user)
    ...
    def get_users_for_diff(self, use_input_diff_collection=prepared_collection, limit=10):
        """
        ...
        if use_input_diff_collection:
            ...
            user_objects = []
            if timedelta:
                more_than_it = datetime.datetime.now() - timedelta
            ...
            if time_start and time_stop:
                user_objects.extend(self.users.find({'$and': [
                        {'date_touch_': {'$gte': time_start}},
                        {'date_touch_': {'$lte': time_stop}}
                ]}, limit=limit))
    ...
    def verify_user(self, name):
        ...
        if user:
            if user['date_touch_'] + min_timedelta > datetime.datetime.now():
